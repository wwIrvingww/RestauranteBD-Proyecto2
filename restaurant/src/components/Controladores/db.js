...
export async function sacartotal(ids) {
    try {
        const result = await client.query(
            `select sum(unitprice)+ (sum(unitprice)*0.11) as total from cuenta 
            join ordenes on  cuenta_id = id
            join alimentos on nombre = alimento
            where id = ${ids}`
        )
        return result.rows
    } catch (err) {
